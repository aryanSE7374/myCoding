...
int f (int n, vector<int>& ht, vector<int>& dp) {
    if ( n<=1 ) {
        return 0;
    }
    if ( n==2 ) {
        return abs (ht[1] - ht[0]);
    }
    if ( dp[n] != -1 ) return dp[n];
    return dp[n] = min ( (f(n-1, ht, dp) + abs(ht[n-1] - ht[n-2])), (f(n-2, ht, dp) + abs(ht[n-1] - ht[n-3])) );
} 
...
int f (int n, vector<int>& ht, vector<int>& dp) {
    ...
    if ( dp[n] != -1 ) return dp[n];
    return dp[n] = min ( (f(n-1, ht, dp) + abs(ht[n] - ht[n-1])), (f(n-2, ht, dp) + abs(ht[n] - ht[n-2])) );
} 
