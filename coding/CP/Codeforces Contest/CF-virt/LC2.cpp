#include<bits/stdc++.h>
using namespace std;

// BFS

// grid -> [color, step]

class Solution {
public:
    
    bool check ( int r, int c, int n, int m) {
        return (
            r>=0 && r<n && c>=0 && c<m
        );
    }
    
    vector<vector<int>> colorGrid(int n, int m, vector<vector<int>>& sources) {

        vector<vector<int>> ans(n , vector<int>(m));
        vector<vector<pair<int, int>>> grid(n , vector<pair<int, int>>(m, {-1, -1})); // temp grid : [color, step]
        
        queue<pair<int, pair<int, pair<int, int> > > > q; // {steps, {color, {x,y}}}

        cout << "\n--- Initial Sources ---\n";
        // step 0 initiially
        for ( auto& src : sources ) {
            int r = src[0], c = src[1], clr = src[2];
            cout << "Source -> (" << r << "," << c << ") Color: " << clr << "\n";
            q.push({0, {clr, {r, c}} });
            grid[r][c] = {clr, 0};
        }

        int dx[4] = {-1, 0, 1, 0}; // U, R, D, L
        int dy[4] = {0, 1, 0, -1};

        cout << "\n--- BFS Start ---\n";

        while ( !q.empty() ) {
            auto it = q.front();
            int steps = it.first;
            int color = it.second.first;
            int x = it.second.second.first;
            int y = it.second.second.second;
            q.pop();

            cout << "Pop -> Step: " << steps 
                 << ", Color: " << color 
                 << ", Cell: (" << x << "," << y << ")\n";

            for ( int i=0; i<4; i++ ) {
                int r = x + dx[i];
                int c = y + dy[i];
                if ( check(r, c, n, m) ) {
                    auto [clr, stp] = grid[r][c];

                    cout << "  Visiting (" << r << "," << c << ") "
                         << "Prev: (clr=" << clr << ", step=" << stp << ")";

                    if ( (clr == -1) || (stp == steps) ) {
                        cout << " -> UPDATE\n";
                        grid[r][c] = {steps, max(clr, color)};
                        q.push( {steps+1, {color, {r, c} } } );

                        cout << "    Pushed -> Step: " << steps+1 
                             << ", Color: " << color 
                             << ", Cell: (" << r << "," << c << ")\n";
                    } else {
                        cout << " -> SKIP\n";
                    }
                }
            }
        }

        cout << "\n--- Final Grid (step stored in first, color in second) ---\n";
        for ( int i=0; i<n; i++ ) {
            for ( int j=0; j<m; j++ ) {
                cout << "(" << grid[i][j].first << "," << grid[i][j].second << ") ";
            }
            cout << "\n";
        }

        for ( int i=0; i<n; i++ ) {
            for ( int j=0; j<m; j++ ) {
                ans[i][j] = grid[i][j].second;
            }
        }

        return ans;
        
    }
};

void printAns(vector<vector<int>>& ans) {
    cout << "\nResult Grid:\n";
    for (auto &row : ans) {
        for (auto &val : row) {
            cout << val << " ";
        }
        cout << "\n";
    }
}

int main() {

    Solution sol;

    // -------- Test Case 1 --------
    cout << "\n================ TEST CASE 1 ================\n";
    int n1 = 3, m1 = 3;
    vector<vector<int>> sources1 = {{0,0,1},{2,2,2}};
    auto ans1 = sol.colorGrid(n1, m1, sources1);
    printAns(ans1);

    // -------- Test Case 2 --------
    cout << "\n================ TEST CASE 2 ================\n";
    int n2 = 3, m2 = 3;
    vector<vector<int>> sources2 = {{0,1,3},{1,1,5}};
    auto ans2 = sol.colorGrid(n2, m2, sources2);
    printAns(ans2);

    // -------- Test Case 3 --------
    cout << "\n================ TEST CASE 3 ================\n";
    int n3 = 2, m3 = 2;
    vector<vector<int>> sources3 = {{1,1,5}};
    auto ans3 = sol.colorGrid(n3, m3, sources3);
    printAns(ans3);

    return 0;
}